...
public class MessageRateLimitingService
{
    ...
    public bool IsRateLimited(ChatId chatId, long userId, string key, out TimeSpan allowedIn)
    {
    ...
    /// <param name="userId">Id of the user.</param>
    /// <param name="key">Additional key to distinguish different limits within a single chat.</param>
    public void LimitRate(ChatId chatId, long userId, string key)
    {
        var requestItemKey = FormatItemKey(chatId, userId, key);
    ...
    private string FormatItemKey(ChatId chatId, long userId, string key) => $"{chatId}_{userId}_{key}";
}
